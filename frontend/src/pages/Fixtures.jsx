...
export default function Fixtures() {
  ...
  return (
    <div className="page-enter max-w-4xl mx-auto px-4 py-8">
      ...
      {!loading && !error && Object.entries(grouped).map(([fecha, items]) => (
        <div key={fecha} className="mb-8">
          <h2 className="font-display text-2xl text-slate-400 mb-3">{fecha}</h2>
      ...
      {!loading && !error && !fixtures?.length && (
        <p className="text-slate-500 text-center py-12">No hay partidos para mostrar</p>
      )}
    </div>
  )
}
