...
export default function StandingsTable({ standings }) {
  if (!standings?.length) return <p className="text-slate-500 text-sm text-center py-6">Sin datos de tabla</p>
        ...
        <tbody>
          {standings.map((s, i) => (
            ...
            >
              ...
              <td className="py-3">
                <Link to={`/teams/${s.team?.id}`} className="font-medium text-white hover:text-green transition-colors">
                  {s.team?.name}
                </Link>
              </td>
              <td className="py-3 text-center text-slate-400">{s.played}</td>
              <td className="py-3 text-center text-slate-400">{s.won}</td>
              <td className="py-3 text-center text-slate-400">{s.drawn}</td>
              <td className="py-3 text-center text-slate-400">{s.lost}</td>
              <td className="py-3 text-center text-slate-400">{s.goals_for}</td>
              <td className="py-3 text-center text-slate-400">{s.goals_against}</td>
              <td className="py-3 text-center text-slate-400">
                {s.goals_for - s.goals_against > 0 ? '+' : ''}{s.goals_for - s.goals_against}
              </td>
              <td className="py-3 text-center font-display text-lg text-green">{s.points}</td>
            </tr>
          ))}
        </tbody>
      </table>
